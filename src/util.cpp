...
const char * const BITCOIN_CONF_FILENAME = "rippedcoin.conf";
const char * const BITCOIN_PID_FILENAME = "rippedcoin.pid";
...
#else
    const char* pszModule = "rippedcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Rippedcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Rippedcoin";
#else
    // Unix
    return pathRet / ".rippedcoin";
#endif
#endif
}
